...
class PatchTrainer(object):
    ...
    def train(self, starter=None):
        """
        Optimize a patch to generate an adversarial example.
        :return: Nothing
        """
        starter = "patches/object_upper.png"
        ...
        if starter:
            adv_patch_cpu = self.read_image(starter)
        else:
            adv_patch_cpu = self.generate_patch("gray")
        #adv_patch_cpu = self.read_image("saved_patches/patchnew6.jpg")
...
def main():
    ...
    if len(sys.argv) == 3:
        trainer = PatchTrainer(sys.argv[1])
        starter = PatchTrainer(sys.argv[2])
        trainer.train()
